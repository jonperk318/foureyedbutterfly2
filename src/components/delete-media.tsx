  ...
  const handleDelete = () => {
    deleteMediaMutation.mutate(selectedFiles, {
      onSuccess: (response) => {
        ...
        toast.success(
          `Successfully deleted ${response.deleted} file${response.deleted === 1 ? "" : "s"}`,
        );
        setSelectedFiles([]);
        mediaQuery.refetch();
      },
  ...
  return (
    <div className={`p-3 md:p-4`}>
      {mediaQuery.data ? (
        <>
          <div
            className={`flex flex-col md:flex-row justify-around md:justify-between items-center h-60 md:h-24`}
          >
            <h1 className={`text-sm lg:text-lg`}>
              Select and delete uploaded images and videos
            </h1>
            <div className={`flex flex-col items-center sm:flex-row gap-4`}>
              ...
              <button
                ...
                disabled={selectedFiles.length === 0}
                onClick={() =>
                  document.getElementById("delete-files-modal")?.showModal()
                }
              >
          ...
          >
            {mediaQuery.data.map((file) => (
              <div
                className={`indicator rounded-box shadow-sm bg-base-100 p-2 md:p-3 w-full items-center justify-center`}
                key={file.name}
              >
                <span className={`indicator-item`}>
                  <input
                    type="checkbox"
                    className={`checkbox checkbox-xl checkbox-accent`}
                    onChange={() => toggleSelectedFiles(file.name)}
                    checked={
                      file.name ? selectedFiles.includes(file.name) : false
                    }
                  />
                </span>
                <Image
                  alt={file.name ? file.name : "Uploaded image/video"}
                  src={`posts/${file.name}`}
                />
              </div>
            ))}
